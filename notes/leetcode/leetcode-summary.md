...
| Clone graph | Graph | DFS the graph while keep updating the map of (origin->clone node). in dfs treat the map as visited list, if exists return node previously cloned, else clone the node and traverse thru each neighbors and do DFS recursively | O(v+e) {node(vertice) + neigbors(edges)} | O(v+e) |
...
| Course schedule | DFS | go thru prerequisite list and create map of (course->prereq list) then create DFS helper to try to run the course if it can be finished by the prereq list (think of it as neighbor map since we need to also check if each prereq course can be finished). then run thru all courses if any stuck at studying stage then we cannot complete all course, return true otherwise | O(v+e) | O(v+e) |
